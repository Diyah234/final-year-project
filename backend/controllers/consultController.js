...
const listConsultByEmail = async (req, res) => {
  ...
  try {
    // Explicitly select the fields you want to return, including patientEmail
    const consultations = await consultModel
      .find({ doctorEmail: email })
      .select(
        "_id doctorId doctorEmail patientName patientEmail bloodGroup hasMedicalConditions medicalConditionDetails allergies consultationReason additionalInfo createdAt"
      );
